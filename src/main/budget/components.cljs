(ns budget.components
  (:require [budget.helpers :refer [classes transaction-id]]
            ...
            ["@heroicons/react/outline" :refer
             [XIcon HomeIcon ChartBarIcon MenuAlt2Icon BellIcon]]
...
(defn container
  [& children]
  [:div {:class (classes "max-w-7xl mx-auto px-4 sm:px-6 lg:px-8")} children])
...
(defn price-input
  [{:keys [id label name on-change value class error]}]
  [:div [:label.block.text-sm.font-medium.text-gray-700 {:for id} label]
   [:div.mt-1.relative.rounded-md.shadow-sm
    ...
    (when error
      [:p#amount-error {:className "absolute -bottom-8 text-sm text-red-600"}
       error])]])

(defn name-input
  [{:keys [id label name placeholder on-change value class]}]
  [:div [:label.block.text-sm.font-medium.text-gray-700 {:for id} label]
   [:div.mt-1.relative.rounded-md.shadow-sm
    [:input
     {:id id
...
(defn button-primary
  [{:keys [size text on-click class disabled?]}]
  (prn disabled?)
...
(defn table
  ...
  [{:keys [head rows]}]
  [table-container
   [:table.min-w-full.divide-y.divide-gray-200 head
    [:tbody.bg-white.divide-y.divide-gray-200 rows]]])
  ...
  [:thead
   [:tr
    [:th.px-6.py-3.bg-gray-50.text-left.text-xs.font-medium.text-gray-500.uppercase.tracking-wider
     "Transaction"]
    [:th.px-6.py-3.bg-gray-50.text-right.text-xs.font-medium.text-gray-500.uppercase.tracking-wider
     "Amount"]
    [:th.hidden.px-6.py-3.bg-gray-50.text-left.text-xs.font-medium.text-gray-500.uppercase.tracking-wider.md:block
     "Status"]
    [:th.px-6.py-3.bg-gray-50.text-right.text-xs.font-medium.text-gray-500.uppercase.tracking-wider
     "Date"]]])
...
(defn recipient-column
  ...
  [:td.max-w-0.w-full.px-6.py-4.whitespace-nowrap.text-sm.text-gray-900
   [:div.flex
    [:a.group.inline-flex.space-x-2.truncate.text-sm {:href "#"} [payment-icon]
     [:p.text-gray-500.truncate.group-hover:text-gray-900
      (str "Payment to " recipient)]]]])
...
(defn status-col
  ...
  [:td.hidden.px-6.py-4.whitespace-nowrap.text-sm.text-gray-500.md:block
   [:span.inline-flex.items-center.px-2.5.py-0.5.rounded-full.text-xs.font-medium.bg-green-100.text-green-800.capitalize
    "success"]])
...
(defn transaction-table-row
  [{:keys [recipient amount time]}]
  [:tr.bg-white [recipient-column recipient] [amount-col amount] [status-col]
   [date-col time]])
    ...
    [chart-responsive-container {:width "100%" :height "100%"}
     [bar-chart
      {:width 500
       ...
       :margin {:top 5 :right 30 :left 20 :bottom 5}} [x-axis {:dataKey "name"}]
      [y-axis {:unit "₴"}] [tooltip {:name "total spending" :unit "₴"}] [legend]
